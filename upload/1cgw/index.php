...
class OneCGateway extends Controller {
  ...
  private function _makeUniqueKeyword($keyword, $query_begin, $current_id) {
  	...
  	$keywords = $this->model_extension_exchange_1c->getAllKeywords($query_begin, $keyword, $current_id);
   ...
   */
	...
	*/
		...
		if (isset($images['image']) && is_array($images['image'])) {
			...
			foreach($images['image'] as $k => $v) {
				if ($images['dir_image'][$k]) {
					if ($v) {
						$this->_saveImage($this->image_dir . $images['dir_image'][$k], $v);
					}

					$buffer[] = array(
						'image' => $this->image_dir . $images['dir_image'][$k],
						'sort_order' => $k
					);
				}
			}
	...
	private function _saveProduct($args, $languages, $lang) {
		...
		if ($args['manufacturer']) {
			$manufacturer_info = $this->model_extension_exchange_1c->getManufacturerByName((string)$args['manufacturer']);
		...
		$option_exchange = $this->model_extension_exchange_1c->getProductOptionValueExchange($product_id);
	...
	private function _saveProductAttributes($product_id, $attributes, $product_attributes) {
		...
		foreach ($attribute_names as $k => $attribute_name) {
			...
			if ($find->num_rows == 0) {
				$attribute_group_id = $this->setting['attribute_group'] ? $this->setting['attribute_group'] : $this->_addGroupAttribute();
	...
	private function _saveProductOptions($product_id, $args, $option_exchange) {
		...
		$this->model_extension_exchange_1c->clearProductOption($product_id);

		$related = $this->model_extension_exchange_1c->existExtension('module', 'related_options'); // checking if module related_option is installed
		...
		foreach ($option_names as $key => $option_name) {
			...
			if (!isset($option_exchange[$id_1c]) && !$find_result) {
				...
				$db->query("insert into " . DB_PREFIX . "product_option (`product_id`, `option_id`, `option_value`, `required`) values ($product_id, $option_id, '', $required);");
				...
				$db->query("insert into " . DB_PREFIX . "product_option (`product_option_id`, `product_id`, `option_id`, `option_value`, `required`) 
			...
			$this->model_extension_exchange_1c->saveProductOptionValueExchange($product_id, $option_id, $option_value_id, $product_option_id, $product_option_value_id, $id_1c);
			$data = array(
				'option_id' => $option_id,
		...
		if ($products) {
			foreach ($products as $product) {
				if ($product) {
					...
					$option_exchange = $this->model_extension_exchange_1c->getProductOptionValueExchange($product['id']);
						...
						if ($product) {
							...
							if (isset($product['options']['option']) && is_array($product['options']['option'])) {
								...
								foreach ($product['options']['option'] as $option_id_1c) {
									if ($option_id_1c) {
										$product_option_value = $this->model_extension_exchange_1c->getProductOptionValueExchange($product['product_id'], $option_id_1c);
										$product_option_data = $this->model_extension_exchange_1c->getProductOptionAndValue($product['product_id'], $product_option_value[$option_id_1c]['product_option_value_id'], $lang);
	...
	* @return string
	*/
	/*public function sendOrderStatuses($args, $signature) {
		if ($this->enable_logs) {
			$fp = fopen( "./log_send_order_statuses.log", "a+" );
			fwrite($fp, var_export($args, true)."\r\n");
			fclose($fp);
		}
		...
		if (!$this->_validateSignature($signature)) {
			if ($this->enable_logs) {
				$fp = fopen( "./log_send_status_order.log", "a+" );
				fwrite($fp, 'Signature is not correct'."\r\n");
				fclose($fp);
			}
			return array('error' => 'Signature is not correct');
		...
		if ($statuses) {
			foreach ($statuses as $status) {
				if ($status) {
					...
					$get_status = $this->model_localisation_order_status->getOrderStatus($status['id']);
					if ($get_status) {
						$status_id = $get_status['order_status_id'];
					// try to find status by name
					} else {
						$get_status = $this->model_extension_exchange_1c->getOrderStatusByName($status['name'], $lang);
						if ($get_status) {
							$status_id = $get_status['order_status_id'];
						}
					}

					$data = array('order_status' => array());
					foreach ($languages as $k => $v) {
						if ($v["language_id"] == $lang) {
							$data['order_status'][$v["language_id"]] = array('name' => (string)$status['name']);
						} else {
							$data['order_status'][$v["language_id"]] = array('name' => $this->_transliterateString((string)$status['name'], false));
						}
					}

					// edit status
					if ($status_id) {
						$this->model_localisation_order_status->editOrderStatus($status_id, $data);
					// add status
					} else {
					...
					$status_ids[$status['id_1c']] = $status_id;
				}
			}
		}

		return array('error' => '', 'status' => $status_ids);
	}*/
